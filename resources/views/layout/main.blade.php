...
<head>
    ...
    <script type="text/javascript" src="{{ asset('public/vendor/jquery-clockpicker/bootstrap-clockpicker.min.js') }}">
    </script>
    ...
    <script type="text/javascript" src="{{ asset('public/vendor/datatable/datatable.responsive.boostrap.min.js') }}">
    </script>
                ...
                <div class="navbar-holder d-flex align-items-center justify-content-between">
                    ...
                    <span class="brand-big" id="site_logo_main">
                        @if ($general_settings->site_logo ?? 'no')
                            <img src="{{ url('public/logo', $general_settings->site_logo ?? 'no') }}"
                                width="50">&nbsp;
                            &nbsp;
                        @endif
                        <h1 class="d-inline" id="site_title_main">{{ $general_settings->site_title ?? 'No title' }}
                        </h1>
                    </span>
                        ...
                        @if (Auth::user()->role_users_id == 1)
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('help') }}" target="_blank"
                                    data-toggle="tooltip" title="{{ __('Help') }}">
    ...
    <nav class="side-navbar">
                    ...
                    @can('core_hr')
                        <li class="has-dropdown {{ request()->is('core_hr*') ? 'active' : '' }}">
                            ...
                            <ul id="Core_hr" class="collapse list-unstyled">
                                ...
                                @can('view-award')
                                    <li id="award"><a
                                            href="{{ route('awards.index') }}">{{ trans('file.Award') }}</a></li>
                                @endcan
                                @can('view-travel')
                                    <li id="travel"><a
                                            href="{{ route('travels.index') }}">{{ trans('file.Travel') }}</a></li>
                                @endcan
                                @can('view-transfer')
                                    <li id="transfer"><a
                                            href="{{ route('transfers.index') }}">{{ trans('file.Transfer') }}</a>
                                    </li>
                                @endcan
                                ...
                                @can('view-warning')
                                    <li id="warning"><a
                                            href="{{ route('warnings.index') }}">{{ trans('file.Warnings') }}</a>
                                    </li>
                                @endcan
                ...
                @can('user')
                    <li
                        ...
                        <ul id="users" class="collapse list-unstyled ">
                            @can('view-user')
                                <li id="users-menu"><a href="{{ route('users-list') }}">{{ __('Users List') }}</a>
                                </li>
                            @endcan
                    ...
                    <li
                        ...
                        @if (auth()->user()->can('view-event') ||
                            ...
                            <a href="#Events_Meetings" aria-expanded="false" data-toggle="collapse"> <i
                                    class="dripicons-to-do"></i><span>{{ trans('file.Events') }} &
                                    {{ trans('file.Meetings') }}</span>
                            </a>
                        @endcan
        ...
        <li class="has-dropdown {{ request()->is('organization*') ? 'active' : '' }}"><a
            ...
            <ul id="Organization" class="collapse list-unstyled ">
                @can('view-company')
                    <li id="company"><a
                            href="{{ route('companies.index') }}">{{ trans('file.Company') }}</a></li>
                @endcan
                @can('view-department')
                    <li id="department"><a
                ...
                @can('view-location')
                    <li id="location"><a
                            href="{{ route('locations.index') }}">{{ trans('file.Location') }}</a></li>
                @endcan
                ...
                <li id="announcements"><a
                        href="{{ route('announcements.index') }}">{{ trans('file.Announcements') }}</a>
                </li>

                <li id="company_policy"><a
                        href="{{ route('policy.index') }}">{{ __('Company Policy') }}</a>
                </li>
        ...
        {{-- @can('timesheet') --}}
        <li class="has-dropdown {{ request()->is('timesheet*') ? 'active' : '' }}"><a
                href="#Timesheets" aria-expanded="false" data-toggle="collapse">
            ...
            <ul id="Timesheets" class="collapse list-unstyled ">
                ...
                @can('view-holiday')
                    <li id="holiday"><a
                            href="{{ route('holidays.index') }}">{{ __('Manage Holiday') }}</a></li>
                @endcan
                @can('view-leave')
                    <li id="leave"><a href="{{ route('leaves.index') }}">{{ __('Manage Leaves') }}</a>
                    </li>
                @endcan
            </ul>
        </li>
        ...
        @can('performance')
            <li class="has-dropdown {{ request()->is('performance*') ? 'active' : '' }}">
                @if (auth()->user()->can('view-goal-type') ||
                ...
                <ul id="performance" class="collapse list-unstyled ">
                    @can('view-goal-type')
                        <li id="goal-type"><a
                                href="{{ route('performance.goal-type.index') }}">{{ __('Goal type') }}</a>
                        </li>
                    @endcan
                    @can('view-goal-tracking')
                        <li id="goal-tracking"><a
                                href="{{ route('performance.goal-tracking.index') }}">{{ __('Goal Tracking') }}</a>
                        </li>
                    @endcan
                    @can('view-indicator')
                        <li id="indicator"><a
                                href="{{ route('performance.indicator.index') }}">{{ __('Indicator') }}</a>
                        </li>
                    @endcan
                    @can('view-appraisal')
                        <li id="appraisal"><a
                                href="{{ route('performance.appraisal.index') }}">{{ __('Appraisal') }}</a>
                        </li>
                    @endcan
                </ul>
            </li>
        ...
        @can('hr_report')
            <li class="has-dropdown {{ request()->is('report*') ? 'active' : '' }}"><a href="#HR_Reports"
                ...
                <ul id="HR_Reports" class="collapse list-unstyled ">
                    ...
                    @endcan
                    @can('report-task')
                        <li id="task_report"><a href="{{ route('report.task') }}">{{ __('Task Report') }}</a>
                        </li>
                    @endcan
                    @can('report-employee')
        ...
        @can('recruitment')
            <li class="has-dropdown {{ request()->is('recruitment*') ? 'active' : '' }}">
                ...
                @if (auth()->user()->can('view-job_post') ||
                ...
                <ul id="Recruitment" class="collapse list-unstyled ">
                    ...
                    @can('view-job_post')
                        <li id="job_post"><a href="{{ route('job_posts.index') }}">{{ __('Job Post') }}</a>
                        </li>
                    @endcan
    ...
    @can('customize-setting')
        <li class="has-dropdown {{ request()->is('settings*') ? 'active' : '' }}">
            ...
            <ul id="Customize_settings" class="collapse list-unstyled ">
                @can('view-role')
                    <li id="roles"><a
                            href="{{ route('roles.index') }}">{{ __('Roles and Access') }}</a></li>
                @endcan
                ...
                @can('view-general-setting')
                    <li id="ip_setting"><a
                            href="{{ route('ip_setting.index') }}">{{ __('IP Settings') }}</a></li>
                @endcan
...
<li class="has-dropdown {{ request()->is('file_manager*') ? 'active' : '' }}">
    ...
    @if (auth()->user()->can('view-file_manager') ||
        auth()->user()->can('view-official_documents'))
...
href={{ $general_settings->footer_link }}
class="external">{{ $general_settings->footer }}</a></p>
</div>
</footer>
</div>
